...
async def get_calendars(request: Request, user: UserModel = Depends(get_admin_user)):
...
async def create_calendar(request: Request, form_data: CalendarForm, user: UserModel = Depends(get_admin_user)):
...
async def create_event(request: Request, form_data: CalendarEventForm, user: UserModel = Depends(get_admin_user)):
    await check_calendar_permission(request, user)
...
async def get_event(request: Request, event_id: str, user: UserModel = Depends(get_admin_user)):
    await check_calendar_permission(request, user)
...
async def update_event(
    request: Request, event_id: str, form_data: CalendarEventUpdateForm, user: UserModel = Depends(get_admin_user)
):
    await check_calendar_permission(request, user)
    event = await CalendarEvents.get_event_by_id(event_id)
    if not event:
...
async def delete_event(request: Request, event_id: str, user: UserModel = Depends(get_admin_user)):
    await check_calendar_permission(request, user)
    event = await CalendarEvents.get_event_by_id(event_id)
    if not event:
        raise HTTPException(status_code=404, detail='Event not found')
...
async def rsvp_event(
    request: Request, event_id: str, form_data: RSVPForm, user: UserModel = Depends(get_admin_user)
):
...
async def get_calendar_by_id(request: Request, calendar_id: str, user: UserModel = Depends(get_admin_user)):
    await check_calendar_permission(request, user)
...
async def update_calendar(
    request: Request, calendar_id: str, form_data: CalendarUpdateForm, user: UserModel = Depends(get_admin_user)
):
    await check_calendar_permission(request, user)
...
async def delete_calendar(request: Request, calendar_id: str, user: UserModel = Depends(get_admin_user)):
    await check_calendar_permission(request, user)
...
async def set_default_calendar(request: Request, calendar_id: str, user: UserModel = Depends(get_admin_user)):
    await check_calendar_permission(request, user)

...
"""
    try:
        ...
        print(f'Durar Ai v{VERSION} - building the best AI user interface.\nhttps://github.com/open-webui/open-webui')
...
@app.post('/api/v1/chat/completions')  # Experimental: Compatibility with OpenAI API
    ...
    # Guest message limit: 5 messages
    if user is None:
        client_ip = request.client.host
        redis = request.app.state.redis
        if redis:
            key = f'guest_chat_count:{client_ip}'
            count = await redis.get(key)
            if count and int(count) >= 5:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail='You have reached the maximum number of messages for guests. Please log in to continue.',
                )
            await redis.incr(key)
            await redis.expire(key, 86400)  # Reset every 24 hours
...
async def generate_messages(
    ...
    All models configured in Durar Ai are accessible via this endpoint.
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for Durar Ai.
